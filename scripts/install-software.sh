...
if [ -f /etc/debian_version ]; then
    ...
    if [ $(cat /etc/lsb-release | grep DISTRIB_RELEASE | cut -d "=" -f 2) == "20.04" ]; then
        sudo apt install -y git kitty tmux zsh rsync htop bat fzf python3 unzip fd-find wget ripgrep neovim
        echo "Please install lf and lsd manually if you want"
    else
        sudo apt install -y git kitty tmux zsh lf rsync htop bat fzf python3 unzip fd-find lsd wget ripgrep neovim
    fi
...
if [ -f /etc/arch-release ]; then
    ...
    sudo pacman -Sy
    sudo pacman -S --needed --noconfirm kitty git tmux zsh lf rsync htop bat python fzf unzip zoxide lsd fd wget ripgrep neovim glow
fi
...
if [ $(uname -s) = "Darwin" ]; then
    ...
    brew update
    brew install git tmux zsh lf kitty rsync htop bat python fzf unzip zoxide lsd fd wget ripgrep neovim glow zathura
fi
